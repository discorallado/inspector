{{--
    Fork de sidebar/group.blade.php: único cambio vs el original de
    vendor/filament/filament — el grupo que contiene el ítem activo
    ($active) arranca siempre expandido, ignorando el estado guardado en
    localStorage (Alpine $store.sidebar.groupIsCollapsed(label)). Pedido
    explícito: en el sub-nav de un cluster (ej. Configuración), si el
    usuario había colapsado ese grupo en una visita anterior, quedaba
    colapsado aunque estuviera parado en un ítem de adentro — confuso.
    Se resuelve interpolando $activeJs (true/false ya resuelto en PHP) antes
    de cada chequeo de colapso: el estado guardado se sigue respetando para
    los grupos NO activos, pero el activo nunca puede quedar colapsado al
    cargar.
--}}
@props([
    'active' => false,
...
@php
    ...
    // json_encode en vez de @js(): dentro de un atributo de componente Blade
    // (x-bind:aria-expanded="..." en <x-filament::icon-button>) @js() no se
    // evalúa — queda como texto literal en el HTML (comprobado). Interpolar
    // el booleano ya resuelto evita ese problema en los tres lugares que lo
    // usan.
...
<li
    ...
    x-bind:class="{ 'fi-collapsed': (! {{ $activeJs }}) && $store.sidebar.groupIsCollapsed(label) }"
    {{
        $attributes->class([
            'fi-sidebar-group',
            'fi-active' => $active,
...
>
        ...
        >
            ...
            @if ($collapsible)
                <x-filament::icon-button
                    ...
                    x-bind:aria-expanded="{{ $activeJs }} || ! $store.sidebar.groupIsCollapsed(label)"
    ...
    <ul
        @if (filled($label))
            ...
            @if ($sidebarCollapsible)
                x-show="$store.sidebar.isOpen ? ({{ $activeJs }} || ! $store.sidebar.groupIsCollapsed(label)) : ! @js($hasDropdown)"
            @else
                x-show="{{ $activeJs }} || ! $store.sidebar.groupIsCollapsed(label)"
            @endif
